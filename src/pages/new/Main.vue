...
export default {
  ...
  async mounted() {
    this.loading = true;
    let response = await GistApi.list(this.query);
    ...
    let res = await GistApi.single(this.blog.id);
    let res2 = res.data;
    for (let key in res2.files) {
      this.blog["title"] = key;
      this.blog["content"] = this.$markdown(res2.files[key]["content"]);
      this.blog["description"] = res2["description"];
      this.blog["createTime"] = this.$util.utcToLocal(res2["created_at"]);
      this.blog["updateTime"] = this.$util.utcToLocal(res2["updated_at"]);
      break;
    }
    this.loading = false
